...
# #     { arn_role_origin_account = "arn:aws:iam::${data.aws_caller_identity.current_session.account_id}:role/tf-role-replication-${each.value.bucket}", arn_destination_bucket = "arn:aws:s3:::${each.value["replication"]["rule"]["destination"]["bucket"]}"
# #   })
